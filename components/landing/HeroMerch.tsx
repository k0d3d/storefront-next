...
const roleTitles = ['Builders', 'Vendors', 'Promoters'];
...
export default function HeroMerch() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveRole(prev => (prev + 1) % roleTitles.length);
    }, 2800);
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#0F0F0F] px-4 sm:px-6 lg:px-8 pt-24">
      ...
      {/* Content Container */}
      <div className="max-w-md sm:max-w-2xl lg:max-w-4xl mx-auto relative z-10 text-center py-12 hero-container">
        {/* Main Headline */}
        <div className="hero-headline flex flex-col">
          <h1 className="text-2xl md:text-4xl font-bold font-mono text-white mb-1 leading-none max-w-sm sm:max-w-none mx-auto">
            Global <br />match-making for
          </h1>
          <p
            className="inline-block text-[#FF4E8C] italic text-6xl sm:text-6xl md:text-[130px] lg:text-[130px] xl:text-[130px] font-exo"
            style={{
          ...
          <p
            className="inline-block text-[#FF4E8C] italic text-3xl sm:text-3xl md:text-4xl"
            style={{
              textShadow: '0 0 40px rgba(255, 78, 140, 0.5)',
              transform: 'rotate(-2deg)'
            }}
          ...
          <div className="relative -top-2 md:-top-6 h-16 sm:h-24 md:h-28 lg:h-32 xl:h-32 flex items-center justify-center overflow-hidden">
            {roleTitles.map((title, index) => (
              <p
                key={title}
                className={`absolute text-5xl sm:text-6xl md:text-7xl lg:text-[130px] xl:text-[130px] font-bold font-exo text-white transition-all duration-700 ease-out ${
                  activeRole === index
                    ? 'opacity-100 translate-y-0'
                    : 'opacity-0 translate-y-6'
                }`}
                style={{
                  textShadow: '0 0 20px rgba(255, 78, 140, 0.5)',
                  transform: 'rotate(-2deg)'
                }}
            ...
            <span className="sr-only">{roleTitles[activeRole]}</span>
          </div>
        </div>
        ...
        <p className="text-lg sm:text-xl text-white/90 max-w-sm sm:max-w-2xl lg:max-w-4xl mx-auto mb-4 leading-relaxed px-2">
          Setup shop in 2 mins. AI designs, creators drive sales, real earnings from day one.
        </p>
        ...
        {/* CTA Buttons */}
        <div className="flex flex-col gap-4 items-center max-w-xs sm:max-w-md mx-auto hidden">
